...
def get_xml_con(file_path):
    """
    获取xml文中的box坐标 即标签内容
    :param file_path: xml文件路径
    :return: [[left_x,up_y,right_x,down_y,class_name],[left_x,up_y,right_x,down_y,class_name],....]
    """
    xml_file_content = open(file_path,'r', encoding='utf-8')
    tree = ET.parse(xml_file_content)
    root = tree.getroot()
    bboxes_list = []
    for obj in root.iter('object'):
        xmlbox = obj.find('bndbox')
        bbox_str = [xmlbox.find('xmin').text, xmlbox.find('ymin').text, xmlbox.find('xmax').text, xmlbox.find('ymax').text]
        bbox_float = list(map(float,bbox_str))
        boxname = obj.find('name').text
        bbox_float.append(boxname)
        bboxes_list.append(bbox_float)
    bboxes_list = sorted(bboxes_list, key=lambda x:(x[0]+x[2])/2) # 按照x轴中心点坐标升序排序
    return bboxes_list
...
def record_info(label_path, writer_tot, writer_type):
    ...
    #### 获取bbox的数据
    this_img_bboxes = get_xml_con(label_path.replace('.txt','.xml'))
    bbox_strs = json.dumps(this_img_bboxes)
    ...
    # 在写总的 标签本
    want_write_con = this_img_label.strip() + "\t" + bbox_strs + '\n'
